...
async def admin_winner(message: Message):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        query = """
            SELECT w.user_id, w.ticket_number, w.code, w.won_at,
                   u.username, u.full_name, t.score
            FROM winners w
            JOIN users u ON w.user_id = u.user_id
            LEFT JOIN tickets t ON w.ticket_number = t.ticket_number
            LIMIT 1
        """
    ...
    uid, t_num, code, won_at, username, full_name, score = winner

    name = f"@{username}" if username else full_name
    score_str = f"{score}/10" if score is not None else "Не определен"

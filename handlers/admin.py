...
async def admin_set_winner(message: Message):
    if message.from_user.id != OWNER_ID:
        return
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT user_id FROM tickets WHERE ticket_number = ?", (t_num,)) as cursor:
            ...
            if not row:
                await message.answer(f"Билет №{t_num} не найден.")
                return
            uid = row[0]
    ...
    await message.answer(f"✅ Победитель установлен: Билет №{t_num}, User ID: {uid}, Код: {win_code}")

...
async def cmd_set_winner(message: Message):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT user_id FROM tickets WHERE ticket_number = ?", (t_num,)) as cursor:
            ...
            if not row:
                await message.answer(f"Билет №{t_num:05d} не найден в базе.")
                return
            u_id = row[0]
    ...
    await message.answer(f"✅ Победитель установлен!\nБилет №{t_num:05d}, User ID: {u_id}\nСекретный код: {win_code}")

    # Отправка сообщения победителю
    win_msg = (
        "🎉 <b>Поздравляем! Твой билет выиграл iPhone 17!</b>\n\n"
    ...
    try:
        await message.bot.send_message(u_id, win_msg, parse_mode="HTML")
        await message.answer("📱 Сообщение победителю отправлено.")
    except Exception as e:
        await message.answer(f"❌ Не удалось отправить сообщение победителю: {e}")
...
async def admin_winner(message: Message):
    ...
    text = (
        ...
        f"Дата установки: {date_str}\n\n"
        f"Инструкция для победителя: написать на alexandr@cbda.ru"
    )

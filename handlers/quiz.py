...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    res_msg = f"Квиз завершён! Твой результат: <b>{score}/10</b>\n"
    if bonus_tickets > 0:
        res_msg += f"🎁 Ты получаешь <b>+{bonus_tickets}</b> бонусных билетов: {', '.join(issued_bonuses)}"
    else:
        res_msg += "Бонусных билетов в этот раз нет, но твой основной билет участвует в розыгрыше!"

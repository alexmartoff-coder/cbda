...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    if bonus_count > 0:
        bonus_str = f"\nТы получаешь <b>+{len(bonus_tickets)}</b> бонусных билетов за отличный результат!"
    else:
        bonus_str = ""
    ...
    msg = (
        ...
        f"Всего билетов за эту попытку: <b>{total_attempt_tickets}</b>{bonus_str}"
    )
    ...
    # Сначала проверяем лимит, чтобы меню было актуальным

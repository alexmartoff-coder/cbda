...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_tickets):
        ...
        if b_num: bonus_nums.append(f"№{b_num:05d}")

    total_attempt_tickets = 1 + len(bonus_nums)

    result_text = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b>\n"
        f"Начислено бонусных билетов: <b>{len(bonus_nums)}</b>\n"
    )
    if bonus_nums:
        result_text += f"Номера бонусных билетов: {', '.join(bonus_nums)}\n"

    result_text += f"\nВсего за эту попытку получено билетов: <b>{total_attempt_tickets}</b>"

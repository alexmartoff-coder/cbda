...
from database.db import (
    ...
    update_ticket_result, issue_bonus_tickets
)
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    if bonus_count > 0:
        ...
        bonus_text = f"\n🎁 Тебе начислено <b>{bonus_count} бонусных билета</b>: {nums_str}"
    else:
        bonus_text = "\nК сожалению, бонусных билетов в этот раз нет."

    msg = (
        f"🏁 <b>Квиз завершён!</b>\n"
        f"Твой результат: <b>{score}/10</b>\n"
        f"Базовый билет: №{t_num:05d}"
        f"{bonus_text}\n\n"
        f"Всего билетов за эту попытку: <b>{1 + bonus_count}</b>"
    )

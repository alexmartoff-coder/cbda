...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_tickets):
        b_num = await issue_ticket(user_id, "bonus")
        if b_num:
            bonus_nums.append(f"№{b_num:05d}")
            await update_ticket_result(b_num, "bonus", 0)

    total_for_attempt = 1 + len(bonus_nums)
    ...
    if bonus_nums:
        msg += f"Твои билеты за эту попытку: №{t_num:05d}, " + ", ".join(bonus_nums)
    else:
        msg += f"Твой билет за эту попытку: №{t_num:05d}"

...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    # Обновляем результат базового билета
    ...
    for _ in range(bonus_tickets_count):
        ...
        if t_num:
            ...
            issued_bonus_tickets.append(f"№{t_num:05d}")

    if bonus_tickets_count > 0:
        bonus_text = f"🎁 Тебе начислено {bonus_tickets_count} бонусных билетов: {', '.join(issued_bonus_tickets)}"
    else:
        bonus_text = "К сожалению, бонусных билетов в этот раз нет (нужно хотя бы 8 правильных ответов)."

    msg = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b>\n"
        f"Базовый билет: №{base_t_num:05d}\n"
        f"{bonus_text}\n\n"
        f"Итого за эту попытку: <b>{1 + bonus_tickets_count}</b> билетов."
    )

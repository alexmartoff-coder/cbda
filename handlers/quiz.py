...
from db.db import (
    ...
    issue_ticket, is_collection_closed, get_total_tickets_count
)
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    bonus_str = ""
    if bonus_tickets:
        formatted_bonus = ", ".join([f"№{b:05d}" for b in bonus_tickets])
        bonus_str = f"\n🎁 <b>Получено бонусных билетов (+{len(bonus_tickets)}):</b> {formatted_bonus}"
    elif score >= 8:
        bonus_str = "\n🎁 Бонусные билеты получены!"
    else:
        bonus_str = "\nК сожалению, для получения бонусов нужно от 8 правильных ответов."
    ...
    result_text = (
        ...
        f"Базовый билет: <b>№{t_num:05d}</b>"
        f"{bonus_str}\n\n"
        f"Всего билетов выдано в системе: <b>{total_tickets}</b>"
    )

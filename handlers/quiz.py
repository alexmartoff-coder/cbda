...
from db.db import (
    get_quiz_session, update_quiz_score, update_quiz_question,
    finish_quiz_session, check_and_trigger_closure, add_user,
    update_ticket_result, issue_ticket, DB_PATH
)
...
async def safe_send_question(bot: Bot, state: FSMContext, user_id: int, q_idx: int):
    ...
    try:
        ...
        logging.error(f"Error safe_send_question: {e}")
...
async def quiz_timer_logic(bot: Bot, state: FSMContext, user_id: int, q_idx: int, msg_id: int):
    try:
        ...
        if current_state == QuizStates.answering and data.get("current_question_index") == q_idx:
            ...
            # Mandatory 3-second delay after explanation
...
async def start_quiz_handler(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logging.error(f"Error start_quiz_handler: {e}")
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_count):
        # Bonus tickets are issued with status='completed' to prevent them from triggering new quiz sessions
        bonus_num = await issue_ticket(user_id, "bonus", status="completed")
        if bonus_num:
            bonus_tickets.append(bonus_num)

    # Base ticket status becomes completed
    ...
    result_text = (
        ...
        f"За этот квиз получено билетов: <b>{total_tickets}</b> (1 базовый"
    )
    if len(bonus_tickets) > 0:
        nums_str = ", ".join([f"№{n:05d}" for n in bonus_tickets])
        result_text += f" + {len(bonus_tickets)} бонусных: {nums_str}"
    else:
        result_text += " + 0 бонусных"
    result_text += ").\n\nЖелаем удачи в розыгрыше iPhone 17!"
    ...
    # Call check_and_trigger_closure before generating the main menu keyboard

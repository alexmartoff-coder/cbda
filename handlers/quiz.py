...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    if bonus_count > 0:
        bt_str = ", ".join([f"№{n:05d}" for n in bonus_tickets])
        msg = (
            f"🏁 <b>Квиз завершён!</b>\n"
            f"Твой результат: <b>{score}/10</b>\n\n"
            f"🎁 Ты получаешь <b>+{bonus_count}</b> бонусных билета(ов): {bt_str}\n"
            f"Итого за эту попытку: {1 + bonus_count} билета(ов)."
        )
    else:
        msg = (
            f"🏁 <b>Квиз завершён!</b>\n"
            f"Твой результат: <b>{score}/10</b>\n\n"
            f"Бонусных билетов за этот результат не положено.\n"
            f"Итого за эту попытку: 1 билет (базовый)."
        )

...
from db.db import (
    ...
    finish_quiz_session, check_and_trigger_closure, update_ticket_result,
    issue_ticket
)
...
async def quiz_timer_logic(bot: Bot, state: FSMContext, user_id: int, q_idx: int, msg_id: int):
    try:
        ...
        if current_state == QuizStates.answering and data.get("current_question_index") == q_idx:
            ...
            await asyncio.sleep(3) # Delay as required
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    result_text = f"🏁 <b>Квиз завершён!</b>\n\nТвой результат: <b>{score}/10</b>\n"
    if bonus_count > 0:
        result_text += f"🎁 Ты получаешь <b>+{bonus_count}</b> бонусных билетов: {', '.join(bonus_tickets)}\n"
    else:
        result_text += "К сожалению, в этот раз бонусных билетов нет. Нужно минимум 8 правильных ответов.\n"

    result_text += f"\nВсего билетов за эту попытку: <b>{1 + bonus_count}</b>"

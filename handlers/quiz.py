...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    bonus_text = ""
    if issued_bonuses:
        bonus_text = f"\n\n🎁 Тебе начислено <b>{bonus_tickets}</b> бонусных билетов: {', '.join(issued_bonuses)}"
    elif score >= 8:
        bonus_text = "\n\nК сожалению, не удалось выдать бонусные билеты."
    else:
        bonus_text = "\n\nБонусных билетов за этот квиз нет (нужно минимум 8 правильных ответов)."

    msg = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b>"
        f"{bonus_text}\n\n"
        "Спасибо за участие! Розыгрыш iPhone 17 состоится скоро в канале @mozgo_boy."
    )

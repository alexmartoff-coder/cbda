...
from database.db import get_quiz_session, update_quiz_score, update_quiz_question, finish_quiz_session, check_and_trigger_closure, add_user, update_ticket_result, issue_ticket
...
async def safe_send_question(bot: Bot, state: FSMContext, user_id: int, q_idx: int):
    ...
    try:
        ...
        logging.error(f"Error in safe_send_question: {e}")
...
async def cmd_resume_pending_quiz(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT ticket_number FROM tickets WHERE user_id = ? AND status = 'issued' LIMIT 1", (user_id,)) as c:
            ...
            if row:
                ...
                await message.answer(
                    f"🎫 Начинаем квиз для билета №{ticket_num:05d}.\n\n"
                ...
                await message.answer("У вас нет билетов, ожидающих квиза.")
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_count):
        t_num = await issue_ticket(user_id, "bonus")
        if t_num:
            bonus_tickets.append(f"№{t_num:05d}")

    await update_ticket_result(base_t_num, "completed", score)
    await finish_quiz_session(user_id)
    await state.clear()

    total_tickets_this_session = [f"№{base_t_num:05d}"] + bonus_tickets

    if bonus_count > 0:
        msg = (
            f"🏁 <b>Квиз завершён!</b>\n\n"
            f"Твой результат: <b>{score}/10</b>\n"
            f"Ты получаешь <b>{bonus_count}</b> бонусных билета(ов)!\n\n"
            f"Билеты за эту попытку: {', '.join(total_tickets_this_session)}"
        )
    else:
        msg = (
            f"🏁 <b>Квиз завершён!</b>\n\n"
            f"Твой результат: <b>{score}/10</b>\n"
            "К сожалению, этого недостаточно для получения бонусных билетов (нужно минимум 8/10).\n\n"
            f"Твой билет за эту попытку: №{base_t_num:05d}"
        )

...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    if bonus_tickets > 0:
        nums_str = ", ".join([f"№{n:05d}" for n in issued_bonus_nums])
        result_text += f"🎁 Поздравляем! Ты получаешь <b>+{bonus_tickets} бонусных билета</b>: {nums_str}"
    else:
        result_text += "В этот раз без бонусов. Попробуй ещё раз!"

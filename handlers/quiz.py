...
from db.db import (
    ...
    check_and_trigger_closure, update_ticket_result, issue_ticket, is_collection_closed
)
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_count):
        ...
        if b_num:
            issued_bonus_nums.append(b_num)

    await update_ticket_result(t_num, "completed", score)
    await finish_quiz_session(user_id)
    await state.clear()

    total_for_attempt = 1 + len(issued_bonus_nums)

    if bonus_count > 0 and issued_bonus_nums:
        bonus_str = ", ".join([f"№{num:05d}" for num in issued_bonus_nums])
        res_msg = (
            f"🏁 <b>Квиз завершён!</b>\n\n"
            f"Твой результат: <b>{score}/10</b> правильных ответов.\n"
            f"🎁 Тебе начислено <b>+{len(issued_bonus_nums)}</b> бонусных билетов: <b>{bonus_str}</b>\n\n"
            f"Всего билетов за эту попытку: <b>{total_for_attempt}</b> (базовый №{t_num:05d} + бонусы)."
        )
    else:
        res_msg = (
            f"🏁 <b>Квиз завершён!</b>\n\n"
            f"Твой результат: <b>{score}/10</b> правильных ответов.\n"
            f"К сожалению, для получения бонусных билетов нужно набрать хотя бы 8 баллов.\n\n"
            f"Всего билетов за эту попытку: <b>1</b> (базовый №{t_num:05d})."
        )

...
async def safe_send_question(bot: Bot, state: FSMContext, user_id: int, q_idx: int):
    ...
    try:
        ...
        # Utilize asyncio.create_task to manage the 30-second timeout per question.
...
async def quiz_timer_logic(bot: Bot, state: FSMContext, user_id: int, q_idx: int, msg_id: int):
    try:
        ...
        if current_state == QuizStates.answering and data.get("current_question_index") == q_idx:
            ...
            # Mandatory 3-second delay before proceeding to the next question or completion
...
async def process_quiz_answer(callback: CallbackQuery, state: FSMContext):
    ...
    # Cancel timer if user answers before the timeout to prevent duplicate processing
    if user_id in active_quiz_timers:
    ...
    # After each quiz answer, the bot displays an explanation with a mandatory 3-second delay
    await asyncio.sleep(3)
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    # 10 правильных → +3 бонусных
    # 9 правильных → +2 бонусных
    # 8 правильных → +1 бонусный
    # меньше 8 → бонусов нет
    bonus_count = 0
    if score == 10:
        bonus_count = 3
    elif score == 9:
    ...
    # Issue bonus tickets sequentially with status='completed' to prevent infinite loop exploits
    bonus_tickets = []
    for _ in range(bonus_count):
        bonus_num = await issue_ticket(user_id, "bonus", status="completed")
        if bonus_num:
            bonus_tickets.append(bonus_num)

    total_tickets_this_attempt = 1 + len(bonus_tickets)

    # Base ticket status becomes 'completed' with score saved
    await update_ticket_result(t_num, "completed", score)
    await finish_quiz_session(user_id)
    await state.clear()

    # check_and_trigger_closure is called within finish_quiz_logic before the main menu keyboard is generated
    # to ensure the UI immediately reflects if the contest has closed due to the latest tickets issued.
    await check_and_trigger_closure(bot)

    # Show final message
    bonus_str = ""
    if len(bonus_tickets) > 0:
        bonus_nums_str = ", ".join([f"№{n:05d}" for n in bonus_tickets])
        bonus_str = f"🎁 Получено бонусных билетов: <b>{len(bonus_tickets)}</b> ({bonus_nums_str})\n"
    else:
        bonus_str = "Бонусных билетов за эту попытку не получено.\n"

    msg = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b> правильных ответов.\n"
        f"{bonus_str}"
        f"Всего билетов за эту попытку: <b>{total_tickets_this_attempt}</b>."
    )

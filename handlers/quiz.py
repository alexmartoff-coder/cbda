...
from db.db import get_quiz_session, update_quiz_score, update_quiz_question, finish_quiz_session, check_and_trigger_closure, update_ticket_result, issue_ticket
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_tickets):
        bn = await issue_ticket(user_id, "bonus")
        if bn:
            bonus_nums.append(f"№{bn:05d}")

    await update_ticket_result(t_num, "completed", score)
    await finish_quiz_session(user_id)
    await state.clear()

    res_msg = f"🏁 <b>Квиз завершён!</b>\n\nТвой результат: <b>{score}/10</b>\n"
    if bonus_tickets > 0:
        res_msg += f"🎁 Ты получаешь <b>{bonus_tickets}</b> бонусных билетов: {', '.join(bonus_nums)}"
    else:
        res_msg += "К сожалению, в этот раз без бонусов. Нужно минимум 8 правильных ответов."

...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_tickets):
        ...
        if bn:
            issued_bonus.append(f"№{bn:05d}")

    await update_ticket_result(t_num, "completed", score)
    await finish_quiz_session(user_id)
    await state.clear()

    total_for_attempt = 1 + len(issued_bonus)

    if bonus_tickets > 0:
        bonus_msg = f"🎁 За отличный результат тебе начислено <b>{len(issued_bonus)}</b> бонусных билетов: {', '.join(issued_bonus)}"
    else:
        bonus_msg = "В этот раз без бонусных билетов. Попробуй ещё раз!"

    msg = (
        f"<b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b>\n"
        f"{bonus_msg}\n\n"
        f"Итого за эту попытку получено билетов: <b>{total_for_attempt}</b>"
    )

...
async def start_quiz_handler(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logging.error(f"Error picking questions: {e}")
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    # Calculate and issue bonus tickets
    bonus_issued = []
    if score == 10:
        for _ in range(3):
            num = await issue_ticket(user_id, "bonus", status="completed")
            if num: bonus_issued.append(num)
    elif score == 9:
        for _ in range(2):
            num = await issue_ticket(user_id, "bonus", status="completed")
            if num: bonus_issued.append(num)
    ...
    total_attempt_tickets = 1 + len(bonus_issued)
    bonus_text = ""
    if len(bonus_issued) > 0:
        ticket_numbers_str = ", ".join([f"№{num:05d}" for num in bonus_issued])
        bonus_text = f"🎁 За отличный результат ты получаешь {len(bonus_issued)} бонусных билетов: {ticket_numbers_str}!\n\n"
    else:
        bonus_text = "В этот раз без бонусных билетов, но твой базовый билет всё равно участвует в розыгрыше!\n\n"

    msg = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b> правильных ответов.\n\n"
        f"{bonus_text}"
        f"Всего билетов за эту попытку: <b>{total_attempt_tickets}</b>"
    )

    # In handlers/quiz.py, check_and_trigger_closure is called within finish_quiz_logic before the main menu keyboard is generated to ensure the UI immediately reflects if the contest has closed due to the latest tickets issued.

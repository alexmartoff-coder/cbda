...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_tickets):
        # Issue bonus tickets with 'completed' status to avoid triggering new quizzes
        num = await issue_ticket(user_id, "bonus", status='completed')
        if num:
            bonus_numbers.append(f"№{num:05d}")

    await update_ticket_result(t_num, "completed", score)
    await finish_quiz_session(user_id)
    await state.clear()

    result_text = f"Квиз завершён! Твой результат: <b>{score}/10</b>\n"
    if bonus_tickets > 0:
        nums_str = ", ".join(bonus_numbers)
        result_text += f"🎉 Ты получаешь <b>+{bonus_tickets}</b> бонусных билетов: {nums_str}"
    else:
        result_text += "К сожалению, этого недостаточно для получения бонусных билетов (нужно минимум 8 правильных ответов)."

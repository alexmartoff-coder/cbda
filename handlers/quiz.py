...
async def safe_send_question(bot: Bot, state: FSMContext, user_id: int, q_idx: int):
    ...
    if user_id in active_quiz_timers:
        ...
        if old_task != current_task and not old_task.done():
            old_task.cancel()
...
async def start_quiz_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if not session or not session[2]:
        await callback.message.answer("У вас нет активной заявки на квиз!")
        return
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_count):
        ...
        if num:
            bonus_ticket_nums.append(num)

    await update_ticket_result(t_num, "completed", score)
    await finish_quiz_session(user_id)
    await state.clear()

    # Call closure check BEFORE menu generation
    await check_and_trigger_closure(bot)

    total_attempt_tickets = 1 + len(bonus_ticket_nums)

    if bonus_count > 0:
        bonus_str = f"➕ Вы получили <b>+{bonus_count}</b> бонусных билета(ов)!"
    else:
        bonus_str = "Бонусные билеты не начислены (требуется от 8 правильных ответов)."

    summary_text = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b> правильных ответов.\n"
        f"Твой базовый билет: №{t_num:05d}\n"
        f"{bonus_str}\n\n"
        f"Всего за эту попытку получено билетов: <b>{total_attempt_tickets}</b>."
    )

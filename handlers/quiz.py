...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    if score == 10:
        for _ in range(3):
            t = await issue_ticket(user_id, "bonus", status="completed")
            if t: bonus_tickets.append(t)
    elif score == 9:
        for _ in range(2):
            t = await issue_ticket(user_id, "bonus", status="completed")
            if t: bonus_tickets.append(t)
    ...
    bonus_text = ""
    if bonus_tickets:
        nums = ", ".join([f"№{n:05d}" for n in bonus_tickets])
        bonus_text = f"\n\n🔥 <b>Великолепно!</b> За твой результат ты получаешь {len(bonus_tickets)} бонусных билетов: {nums}"

    total_earned = 1 + len(bonus_tickets)

    msg = (
        f"<b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b>\n"
        f"Всего билетов за эту попытку: <b>{total_earned}</b>"
        f"{bonus_text}"
    )

...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_count):
        t_num = await issue_ticket(user_id, "bonus", status="valid")
        if t_num:
            bonus_tickets.append(f"№{t_num:05d}")

    await update_ticket_result(base_t_num, "valid", score)
    await finish_quiz_session(user_id)
    await state.clear()

    if bonus_count > 0:
        bonus_text = f"🎁 <b>Твои бонусные билеты:</b> {', '.join(bonus_tickets)}"
    else:
        bonus_text = "В этот раз без бонусов, но твой базовый билет участвует в розыгрыше!"

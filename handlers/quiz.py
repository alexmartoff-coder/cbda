...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    bonus_msg = ""
    if bonus_count > 0:
        bonus_msg = f"🎁 Ты получаешь <b>+{bonus_count}</b> бонусных билетов: {', '.join(bonus_tickets)}"
    else:
        bonus_msg = "К сожалению, в этот раз без бонусов. Нужно минимум 8 правильных ответов."
    ...
    # Важно: сначала проверяем закрытие, потом получаем клавиатуру

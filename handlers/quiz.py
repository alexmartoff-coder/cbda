...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_tickets):
        ...
        if b_num:
            bonus_nums.append(b_num)

    await update_ticket_result(t_num, "completed", score)
    await finish_quiz_session(user_id)
    await state.clear()

    total_session_tickets = 1 + bonus_tickets

    result_text = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Ваш результат: <b>{score}/10</b>\n"
        f"Начислено бонусных билетов: <b>{bonus_tickets}</b>\n"
        f"Всего билетов за эту попытку: <b>{total_session_tickets}</b>"
    )

    if bonus_nums:
        nums_str = ", ".join([f"№{n:05d}" for n in bonus_nums])
        result_text += f"\nНомера бонусных билетов: {nums_str}"

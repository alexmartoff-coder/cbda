...
async def process_quiz_answer(callback: CallbackQuery, state: FSMContext):
    ...
    # 3-second mandatory delay before proceeding
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    # Calculate bonus tickets and award them sequentially
    bonus_count = 0
    if score == 10:
        bonus_count = 3
    elif score == 9:
    ...
    if bonus_count > 0:
        bonus_numbers_str = ", ".join([f"№{num:05d}" for num in bonus_tickets])
        msg = (
            f"🎉 <b>Квиз завершён!</b>\n\n"
            f"Твой результат: <b>{score}/10</b> правильных ответов.\n"
            f"За отличный результат тебе начислено <b>+{bonus_count}</b> бонусных билетов: {bonus_numbers_str}!\n\n"
        ...
        msg = (
            f"🎉 <b>Квиз завершён!</b>\n\n"
            f"Твой результат: <b>{score}/10</b> правильных ответов.\n"
            f"Для получения бонусов нужно ответить правильно минимум на 8 вопросов.\n\n"
            f"Всего за эту попытку получено билетов: <b>1</b> (базовый билет №{t_num:05d})."
        )

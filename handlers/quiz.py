...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    base_str = f"Базовый билет №{t_num:05d} получен ✅"
    if bonus_numbers:
        bonus_str = ", ".join([f"№{num:05d}" for num in bonus_numbers])
        bonus_info = f"\n• Бонусные билеты: <b>{bonus_str}</b> за отличный результат!"
    else:
        bonus_info = ""

    msg = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Результат: <b>{score}/10</b> правильных ответов.\n"
        f"Тебе начислено бонусных билетов: <b>+{bonus_count}</b>\n\n"
        f"Всего за эту попытку получено билетов: <b>{1 + bonus_count}</b>\n"
        f"• {base_str}{bonus_info}"
    )

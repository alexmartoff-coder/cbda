...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_count):
        ...
        if num: bonus_nums.append(f"№{num:05d}")

    await update_ticket_result(base_t_num, "completed", score)
    await finish_quiz_session(user_id)
    await state.clear()

    total_tickets_this_round = 1 + bonus_count
    res_msg = (
        f"🏁 <b>Квиз завершён!</b>\n"
        f"Ваш результат: <b>{score}/10</b>\n\n"
    )
    if bonus_count > 0:
        res_msg += f"🔥 Потрясающе! Вы получили +{bonus_count} бонусных билета: {', '.join(bonus_nums)}\n"
    else:
        res_msg += "Бонусных билетов в этот раз нет (нужно хотя бы 8 правильных ответов).\n"

    res_msg += f"\nИтого за эту попытку: <b>{total_tickets_this_round}</b> билета."

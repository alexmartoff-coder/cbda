...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_tickets):
        ...
        if t_num:
            issued_bonuses.append(f"№{t_num:05d}")

    msg = f"🏁 <b>Квиз завершён!</b>\n\nТвой результат: <b>{score}/10</b>\n"
    if bonus_tickets > 0:
        msg += f"🎁 Ты получаешь <b>{bonus_tickets}</b> бонусных билета: {', '.join(issued_bonuses)}"
    else:
        msg += "К сожалению, в этот раз без бонусов. Нужно минимум 8 правильных ответов."

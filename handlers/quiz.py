...
from db.db import (
    ...
    update_ticket_result, issue_ticket
)
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    # Bonus logic
    bonus_tickets = 0
    if score == 10: bonus_tickets = 3
    elif score == 9: bonus_tickets = 2
    elif score == 8: bonus_tickets = 1

    bonus_nums = []
    for _ in range(bonus_tickets):
        num = await issue_ticket(user_id, "bonus", status='completed')
        if num:
            bonus_nums.append(f"№{num:05d}")

    total_tickets_this_session = 1 + len(bonus_nums)

    result_text = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b>\n"
        f"Получено бонусных билетов: <b>{len(bonus_nums)}</b>\n"
    )

    if bonus_nums:
        result_text += f"Номера бонусных билетов: {', '.join(bonus_nums)}\n"

    result_text += f"\nВсего билетов за эту попытку: <b>{total_tickets_this_session}</b>"

...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_count):
        bt_num = await issue_ticket(user_id, "bonus", status="active")
        if bt_num:
    ...
    bonus_text = ""
    if bonus_count > 0:
        bonus_text = f"🎁 За результат {score}/10 тебе начислено {bonus_count} бонусных билета: {', '.join(bonus_tickets)}\n\n"
    else:
        bonus_text = f"Твой результат: {score}/10. Бонусных билетов нет.\n\n"

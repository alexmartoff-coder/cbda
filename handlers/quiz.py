...
async def start_quiz_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if not session or not session[2]:
        await callback.message.answer("У вас нет активного билета!")
        return
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_count):
        # status='completed' to prevent bonus tickets from triggering new quizzes
        b_num = await issue_ticket(user_id, "bonus", status='completed')
        if b_num:
            bonus_tickets.append(f"№{b_num:05d}")

    await update_ticket_result(t_num, "completed", score)
    await finish_quiz_session(user_id)
    await state.clear()

    total_tickets_in_attempt = 1 + len(bonus_tickets)

    msg = f"Квиз завершён! Твой результат: <b>{score}/10</b>.\n"
    if bonus_count > 0:
        msg += f"🎉 Ты получаешь <b>+{bonus_count} бонусных билета</b>: {', '.join(bonus_tickets)}.\n"
    else:
        msg += "К сожалению, этого недостаточно для получения бонусных билетов (нужно минимум 8 правильных ответов).\n"

    msg += f"\nВсего за эту попытку получено билетов: <b>{total_tickets_in_attempt}</b>."

...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    from db.db import issue_ticket as db_issue_ticket
    issued_bonuses = []
    for _ in range(bonus_count):
        b_num = await db_issue_ticket(user_id, "bonus", status='completed')
        if b_num:
            issued_bonuses.append(f"№{b_num:05d}")

    total_tickets = 1 + len(issued_bonuses)

    if bonus_count > 0:
        bonus_msg = f"🎁 Твой результат: <b>{score}/10</b>. Ты получаешь <b>+{len(issued_bonuses)} бонусных билета(ов)</b>: {', '.join(issued_bonuses)}!"
    else:
        bonus_msg = f"Твой результат: <b>{score}/10</b>. К сожалению, этого недостаточно для бонусов (нужно минимум 8 правильных)."

    final_msg = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"{bonus_msg}\n\n"
        f"🎫 Итого за эту попытку получено билетов: <b>{total_tickets}</b>\n"
        f"Все твои билеты участвуют в розыгрыше iPhone 17!"
    )

...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    total_for_attempt = 1 + bonus_count

    msg = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b>\n"
        f"Начислено бонусных билетов: <b>{bonus_count}</b>\n\n"
    )

    if bonus_tickets:
        msg += "Твои новые билеты: " + ", ".join([f"№{tn:05d}" for tn in bonus_tickets]) + "\n"

    msg += f"Всего за эту попытку получено билетов: <b>{total_for_attempt}</b>"

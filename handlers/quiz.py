...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    if bonus_count > 0:
        tickets_str = ", ".join(bonus_tickets)
        msg = (
            f"🏁 <b>Квиз завершён!</b>\n\n"
            f"Твой результат: <b>{score}/10</b>\n"
            f"Ты получаешь <b>+{bonus_count} бонусных билета</b>: {tickets_str}\n\n"
            f"Всего за эту попытку получено билетов: <b>{bonus_count + 1}</b>"
        )
    else:
        msg = (
            f"🏁 <b>Квиз завершён!</b>\n\n"
            f"Твой результат: <b>{score}/10</b>\n"
            "Бонусных билетов за этот результат не положено.\n\n"
            f"Всего за эту попытку получен 1 базовый билет №{base_t_num:05d}"
        )

    # Важно вызвать check_and_trigger_closure ДО генерации клавиатуры, чтобы она была актуальной

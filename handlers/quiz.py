...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    if bonus_tickets > 0:
        ...
        for _ in range(bonus_tickets):
            ...
            if b_num:
                issued_bonus.append(f"№{b_num:05d}")

    status = "completed"
    bonus_text = f"🎁 Тебе начислено <b>{len(issued_bonus)}</b> бонусных билетов: {', '.join(issued_bonus)}" if issued_bonus else "К сожалению, в этот раз без бонусов."

    msg = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b>\n\n"
        f"{bonus_text}\n\n"
        f"Все твои билеты (базовый №{t_num:05d} + бонусы) участвуют в розыгрыше iPhone 17!"
    )
    ...
    # Сначала проверяем лимит, потом клавиатуру

...
async def process_quiz_answer(callback: CallbackQuery, state: FSMContext):
    ...
    # The requirement says "After each answer, an explanation... Transition to the next question."
    # We already show explanation in res_text.
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    # Подсчитываются бонусные билеты:
    ...
    total_for_session = 1 + len(bonus_tickets)

    if bonus_count > 0:
        bonus_msg = f"Твой результат: {score}/10. Тебе начислено {len(bonus_tickets)} бонусных билетов!"
        if bonus_tickets:
            bonus_msg += f"\nНомера бонусных билетов: " + ", ".join([f"№{bt:05d}" for bt in bonus_tickets])
    else:
        bonus_msg = f"Твой результат: {score}/10. Бонусных билетов не начислено."

    final_msg = (
        f"<b>Квиз завершён!</b>\n\n"
        f"За эту попытку ты получил всего билетов: {total_for_session}\n"
        f"{bonus_msg}"
    )

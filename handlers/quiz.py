...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    from db.db import issue_bonus_tickets, update_ticket_result, finish_quiz_session
    bonus_tickets = await issue_bonus_tickets(user_id, bonus_count)

    await update_ticket_result(base_t_num, "completed", score)
    await finish_quiz_session(user_id)
    await state.clear()

    all_tickets = [base_t_num] + bonus_tickets
    tickets_str = ", ".join([f"№{t:05d}" for t in all_tickets])

    res_msg = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b>\n"
        f"Получено бонусных билетов: <b>{bonus_count}</b>\n\n"
        f"Твои билеты за этот раунд: {tickets_str}\n\n"
        "Все они примут участие в розыгрыше iPhone 17!"
    )

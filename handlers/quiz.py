...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    # Calculate bonus tickets: 10 -> +3, 9 -> +2, 8 -> +1, <8 -> 0
    bonus_count = 0
    if score == 10:
        bonus_count = 3
    elif score == 9:
    ...
    for _ in range(bonus_count):
        ...
        if b_num:
            await update_ticket_result(b_num, "completed", 0)
            bonus_tickets.append(b_num)
    ...
    # Trigger closure check and potential broadcasting
    await check_and_trigger_closure(bot)

    bonus_text = ""
    if bonus_count > 0:
        tickets_str = ", ".join([f"№{num:05d}" for num in bonus_tickets])
        bonus_text = f"🎁 Тебе начислено <b>{bonus_count}</b> бонусных билетов: {tickets_str}!\n\n"
    else:
        bonus_text = "К сожалению, в этот раз без бонусных билетов.\n\n"

    total_attempt_tickets = [t_num] if t_num else []
    total_attempt_tickets.extend(bonus_tickets)
    total_attempt_str = ", ".join([f"№{num:05d}" for num in total_attempt_tickets])

    msg = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b> правильных ответов.\n"
        f"{bonus_text}"
        f"Всего билетов за эту попытку: <b>{len(total_attempt_tickets)}</b> ({total_attempt_str})."
    )

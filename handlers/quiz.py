...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    from db.db import issue_ticket, update_ticket_result, finish_quiz_session, check_and_trigger_closure
    ...
    if bonus_count > 0 and issued_bonus_tickets:
        bonus_str = ", ".join([f"№{num:05d}" for num in issued_bonus_tickets])
        msg += f"🎁 Ты получаешь <b>+{len(issued_bonus_tickets)} бонусных билета</b>: {bonus_str}\n"
    else:
        msg += "Бонусные билеты не начислены (нужно минимум 8/10).\n"

    msg += f"\nВсего билетов за эту попытку: <b>{total_tickets_this_attempt}</b>"

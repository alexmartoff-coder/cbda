...
from database.db import (
    ...
    check_and_trigger_closure, add_user, update_ticket_result, issue_ticket, mark_questions_as_seen, is_collection_closed
)
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    bonus_tickets = []
    # Перед выдачей каждого бонусного билета проверяем, не закрыт ли сбор
    ...
    total_tickets = [f"№{base_t_num:05d}"] + bonus_tickets
    bonus_text = f"\nТы получил бонусные билеты: {', '.join(bonus_tickets)}" if bonus_tickets else "\nБонусных билетов за этот результат не начислено."

    final_msg = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b>\n"
        f"{bonus_text}\n\n"
        f"Всего билетов за эту попытку: <b>{len(total_tickets)}</b>\n"
        f"Номера билетов: {', '.join(total_tickets)}"
    )

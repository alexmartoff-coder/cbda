...
from db.db import (
    ...
    check_and_trigger_closure, add_user, update_ticket_result, issue_ticket, is_collection_closed
)
...
async def quiz_timer_logic(bot: Bot, state: FSMContext, user_id: int, q_idx: int, msg_id: int):
    try:
        await asyncio.sleep(30) # 30 seconds timer per question
        data = await state.get_data()
...
async def start_quiz_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if not session or not session[2]:
        await callback.message.answer("У вас нет активного билета для прохождения квиза!")
        return
    ...
    try:
        ...
        logging.error(f"Error in start_quiz_handler: {e}")
...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    for _ in range(bonus_count):
        # Bonus tickets are issued with status='completed' to prevent infinite ticket loop exploit
        bonus_t_num = await issue_ticket(user_id, "bonus", status="completed")
        if bonus_t_num:
            # We record score for bonus tickets as 0 or empty, or we can just leave it
            await update_ticket_result(bonus_t_num, "completed", 0)
            bonus_tickets.append(bonus_t_num)

    await finish_quiz_session(user_id)
    await state.clear()

    total_tickets_this_attempt = 1 + len(bonus_tickets)
    base_ticket_str = f"№{t_num:05d}" if t_num else "неизвестно"
    if bonus_tickets:
        bonus_tickets_str = ", ".join([f"№{bt:05d}" for bt in bonus_tickets])
        bonus_msg_part = f"\n🎁 Начислено бонусных билетов: <b>{len(bonus_tickets)}</b> ({bonus_tickets_str})"
    else:
        bonus_msg_part = "\nБонусных билетов за эту попытку нет."

    final_msg = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Твой результат: <b>{score}/10</b> правильных ответов.\n"
        f"Базовый билет: <b>{base_ticket_str}</b>"
        f"{bonus_msg_part}\n\n"
    ...
    # Check and trigger closure is called within finish_quiz_logic before the main menu keyboard is generated

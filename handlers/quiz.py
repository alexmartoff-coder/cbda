...
async def finish_quiz_logic(bot: Bot, state: FSMContext, user_id: int):
    ...
    bonus_str = ""
    if bonus_tickets:
        nums_formatted = ", ".join(f"№{num:05d}" for num in bonus_tickets)
        bonus_str = f"\n🎁 Бонусные билеты ({len(bonus_tickets)} шт.): {nums_formatted}"

    msg = (
        f"🏁 <b>Квиз завершён!</b>\n\n"
        f"Результат: <b>{score}/10</b>\n"
        f"Базовый билет: <b>№{t_num:05d}</b>{bonus_str}\n\n"
        f"Всего билетов за эту попытку: <b>{total_for_attempt}</b>"
    )

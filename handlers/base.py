...
async def cmd_start(message: Message):
    ...
    if not await has_accepted_rules(user_id):
        ...
        await message.answer(f"Используйте меню для навигации.", reply_markup=kb, parse_mode="HTML")
        return
    ...
    await message.answer(
        ...
        "Участвуй в квизе и выигрывай iPhone 17!\n\n"
        f"{progress}",
        reply_markup=kb,
        parse_mode="HTML"
    )
...
async def cmd_rules(message: Message):
    rules_html = (
        ...
        "3. После оплаты ты проходишь квиз из 10 вопросов.\n"
        "4. Бонусные билеты:\n"
        "   — 10 верных: +3 билета\n"
        "   — 9 верных: +2 билета\n"
        "   — 8 верных: +1 билет\n"
        "5. Сбор билетов до 2500 билетов или до 10 апреля 2026.\n"
        "6. Розыгрыш через https://www.random.org/ в канале @mozgo_boy.\n\n"
    ...
    if not apps:
        ...
        for t_num, status, score in apps:
            if status == "pending":
                ...
                score_text = f" (Результат квиза: {score}/10)"
...
async def cmd_leaderboard(message: Message):
    ...
    for i, (username, full_name, ticket_count) in enumerate(leaders, 1):
        ...
        text += f"{i}. {name} — <b>{ticket_count}</b> билета(ов)\n"

...
async def cmd_play_quiz(message: Message):
    user_id = message.from_user.id
    if not await has_accepted_rules(user_id):
        await message.answer("Пожалуйста, примите правила конкурса (/start).")
        return
    ...
    mechanics_text = (
        "<b>Механика игры:</b>\n\n"
        "1. Оплатите 99 ₽ — вы сразу получите <b>1 базовый билет</b>.\n"
        "2. Пройдите квиз из 10 вопросов.\n"
        "3. Получите бонусные билеты за правильные ответы:\n"
        "   ✅ 10 ответов — <b>+3 билета</b>\n"
        "   ✅ 9 ответов — <b>+2 билета</b>\n"
        "   ✅ 8 ответов — <b>+1 билет</b>\n\n"
...
async def cmd_rules(message: Message):
    rules_html = (
        ...
        "1. Стоимость участия — 99 ₽ за одну попытку.\n"
        "2. В одну попытку входит 1 гарантированный билет и до 3 бонусных за квиз.\n"
        "3. Лимит сбора — 2500 билетов или до 10 апреля 2026 года.\n"
        "4. Победитель выбирается случайным образом через random.org.\n\n"
...
async def cmd_my_tickets(message: Message):
    ...
    if not apps:
        ...
        for t_num, status, score in apps:
            text += f"🎟 <b>№{t_num:05d}</b>\n"

        text += "\nВсе эти билеты участвуют в розыгрыше!"

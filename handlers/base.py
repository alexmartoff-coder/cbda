...
async def cmd_start(message: Message):
    ...
    await message.answer(
        f"<b>Добро пожаловать в интеллектуальный квиз «iPhone 17 PRO 256 Гб»!</b>\n\n"
        "Каждый платёж (99 ₽) даёт 1 базовый билет и возможность получить до +3 бонусных билетов за результат в квизе.\n\n"
        f"{progress}",
        reply_markup=kb,
        parse_mode="HTML"
    )
...
async def accept_rules_handler(callback: CallbackQuery):
    ...
    await callback.message.answer(
        ...
        "Оплачивайте участие, получайте билеты и выигрывайте iPhone 17!\n\n"
        f"{progress}",
        reply_markup=kb,
        parse_mode="HTML"
    )
...
async def cmd_rules(message: Message):
    rules_html = (
        ...
        "<b>Лимит билетов:</b> 2500 (две тысячи пятьсот).\n"
        "<b>Старт сбора:</b> 20 марта 2026 г.\n"
        "<b>Окончание сбора:</b> автоматически при достижении 2500 билетов или 10 апреля 2026 г.\n"
        "<b>Определение победителя:</b> честный розыгрыш среди всех выданных билетов через random.org.\n\n"

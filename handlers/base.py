...
async def cmd_start(message: Message):
    ...
    await message.answer(
        f"<b>Добро пожаловать в квиз с розыгрышем iPhone 17!</b>\n\n"
        f"{progress}",
        reply_markup=kb,
        parse_mode="HTML"
    )
...
async def cmd_rules(message: Message):
    rules_html = (
        ...
        "Каждый платёж 99 ₽ даёт 1 гарантированный базовый билет + возможность получить до +3 бонусных билетов за результат в квизе:\n"
        ...
        "Розыгрыш проводится честно среди всех выданных билетов на https://www.random.org/\n\n"
...
async def cmd_my_tickets(message: Message):
    ...
    if not apps:
        await message.answer("У тебя пока нет билетов. Начни квиз в главном меню!")
    else:

...
async def cmd_start(message: Message):
    ...
    if not await has_accepted_rules(user_id):
        agreement_text = (
            "Добро пожаловать в интеллектуальный конкурс за iPhone 17!\n\n"
    ...
    await message.answer(
        ...
        "Участвуй в квизе, проверяй свои знания об Apple и выигрывай iPhone 17.\n\n"
        f"{progress}",
        reply_markup=kb,
        parse_mode="HTML"
    )
...
async def cmd_rules(message: Message):
    rules_html = (
        ...
        "<b>Тематика квиза:</b> компания Apple, её устройства и история.\n"
        "<b>Приз:</b> iPhone 17.\n"
        "<b>Стоимость участия:</b> 99 ₽.\n"
        "<b>Лимит билетов:</b> 2500 (всего).\n"
        "<b>Окончание сбора:</b> при достижении 2500 билетов или 10 апреля 2026 г.\n\n"
        "<b>Механика:</b>\n"
        "1. Каждый платеж дает 1 базовый билет.\n"
        "2. После оплаты запускается квиз (10 вопросов).\n"
        "3. Бонусные билеты:\n"
        "   - 10 верных: +3 билета\n"
        "   - 9 верных: +2 билета\n"
        "   - 8 верных: +1 билет\n\n"
        "Победитель выбирается с помощью random.org среди всех выданных номеров билетов.\n\n"
    ...
    if not apps:
        ...
        for t_num, status, score in apps:
            if status == "pending":
                ...
                status_text = f"✅ Активен (Результат: {score}/10)" if score is not None else "✅ Активен"

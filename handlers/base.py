...
async def cmd_rules(message: Message):
    rules_html = (
        ...
        "1. Каждый платёж в размере 99 ₽ даёт вам 1 базовый билет.\n"
        "2. После оплаты вы можете пройти квиз из 10 вопросов.\n"
        "3. За хорошие результаты в квизе начисляются бонусные билеты:\n"
        "   — 10 правильных ответов: +3 билета\n"
        "   — 9 правильных ответов: +2 билета\n"
        "   — 8 правильных ответов: +1 билет\n"
        "4. Сбор билетов останавливается при достижении 2500 билетов или 10 апреля 2026 г.\n"
        "5. Победитель будет выбран случайным образом через random.org среди всех выданных билетов.\n\n"
        "Полные правила: <a href='https://cbda.ru/rules/base'>https://cbda.ru/rules/base</a>"
    )
    ...
    if not apps:
        ...
        for t_num, status, score in apps:
            if status == "pending":
                ...
                status_text = f"✅ Активен (результат квиза: {score if score is not None else 0}/10)"

...
async def cmd_start(message: Message):
    ...
    if not await has_accepted_rules(user_id):
        agreement_text = (
            "Добро пожаловать в квиз за iPhone 17!\n\n"
    ...
    await message.answer(
        ...
        "Оплачивай квиз (99 ₽), получай гарантированный билет и до +3 бонусных билетов за правильные ответы!\n\n"
        f"{progress}",
        reply_markup=kb,
        parse_mode="HTML"
    )
...
async def cmd_rules(message: Message):
    rules_html = (
        ...
        "• Стоимость одного участия в квизе составляет 99 ₽.\n"
        "• При оплате пользователь получает 1 гарантированный базовый билет.\n\n"
        "<b>2. Бонусные билеты:</b>\n"
        "• 10 правильных ответов out of 10: <b>+3 бонусных билета</b>\n"
        "• 9 правильных ответов out of 10: <b>+2 бонусных билета</b>\n"
        "• 8 правильных ответов out of 10: <b>+1 бонусный билет</b>\n"
        "• Менее 8 правильных ответов: бонусные билеты не начисляются.\n\n"
        "<b>3. Окончание приёма и розыгрыш:</b>\n"
        "• Сбор билетов останавливается при достижении 2500 билетов или 10 апреля 2026 г.\n"
        "• Победитель определяется случайным образом среди всех выданных билетов на сайте https://www.random.org/\n\n"
...
async def cmd_my_tickets(message: Message):
    ...
    if not apps:
        await message.answer("У тебя пока нет билетов. Оплати 99 ₽ для участия в квизе!")
    else:
        text = f"<b>Твои билеты ({len(apps)} шт.):</b>\n\n"
        for t_num, status, score in apps:
            if status == "pending":
                status_text = "⏳ Квиз не пройден"
            elif status == "completed":
                status_text = "✅ Действителен для розыгрыша"
            else:
                status_text = "✅ Действителен для розыгрыша"

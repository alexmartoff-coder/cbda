...
async def show_mechanics(message: Message):
    ...
    mechanics_text = (
        ...
        "1. Ты оплачиваешь <b>99 ₽</b>.\n"
        "2. Получаешь <b>1 гарантированный базовый билет</b>.\n"
        "3. Проходишь квиз из 10 вопросов.\n"
        "4. За хорошие результаты получаешь бонусные билеты:\n"
...
async def start_payment(callback: CallbackQuery):
    ...
    if await is_collection_closed():
        await callback.message.answer("🎉 Приём билетов окончен!")
        return

    try:
        await callback.message.answer_invoice(
            title="Билет + Квиз на iPhone 17",
...
async def successful_payment_handler(message: Message):
    ...
    if ticket_num:
        ...
        await message.answer(f"✅ Оплата прошла! Твой базовый билет <b>№{ticket_num:05d}</b> получен.", parse_mode="HTML")

...
async def start_play_flow(message: Message):
    ...
    text = (
        ...
        "• 9/10 правильных → +2 билета\n"
        "• 8/10 правильных → +1 билет\n\n"
...
async def successful_payment_handler(message: Message):
    ...
    # Выдаем ГАРАНТИРОВАННЫЙ базовый билет
    ticket_num = await issue_ticket(user_id, "paid", status="valid")
    if ticket_num:
        ...
        warning_text = (
            "🚀 <b>Пора получить бонусные билеты!</b>\n\n"
            "Сейчас начнется квиз из 10 вопросов. У тебя есть 30 секунд на каждый ответ.\n\n"
            "⚠️ <b>Внимание!</b> Выбирай время и место с хорошим интернетом. Если закроешь приложение, ответ не засчитается.\n\n"
            "Готов?"
        )

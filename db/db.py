...
async def get_user_ticket_counts(user_id):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT type FROM tickets WHERE user_id = ?", (user_id,)) as cursor:
            rows = await cursor.fetchall()
            total = len(rows)
            base_count = sum(1 for (t_type,) in rows if t_type == 'base')
...
async def check_and_trigger_closure(bot: Bot):
    ...
    now = get_moscow_now().replace(tzinfo=None)
    deadline = datetime.fromisoformat(CONTEST_DEADLINE)
    is_closed_by_limit = total_tickets >= TICKET_LIMIT
    is_closed_by_deadline = now >= deadline

    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT value FROM settings WHERE key = 'is_closure_recorded'") as cursor:
            row = await cursor.fetchone()
            recorded = row[0] == '1' if row else False

    if (is_closed_by_limit or is_closed_by_deadline) and not recorded:

...
async def get_user_ticket_counts(user_id):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT type FROM tickets WHERE user_id = ?", (user_id,)) as cursor:
            rows = await cursor.fetchall()
            total = len(rows)
            # In new version, we don't have free attempts in the same way,
            # but let's keep it for compatibility if needed.
            # Actually, all are considered tickets for the draw.
            return total, 0

async def check_and_trigger_closure(bot: Bot):
    total_tickets = await get_total_tickets_count()
    deadline = datetime.strptime(CONTEST_DEADLINE, "%Y-%m-%d %H:%M:%S")
    is_deadline_passed = get_moscow_now().replace(tzinfo=None) >= deadline

    is_closed = await is_collection_closed()

    if (total_tickets >= TICKET_LIMIT or is_deadline_passed) and not is_closed:
        ...
        async def broadcast_closure():
            if total_tickets >= TICKET_LIMIT:
                ...
                push_text = (
                    ...
                    "Приём билетов окончен по достижении даты 10 апреля. Спасибо всем участникам!\n\n"
                    "Розыгрыш iPhone 17 состоится в ближайшее время в прямом эфире в канале @mozgo_boy. "
                    "Следи за обновлениями!"
                )

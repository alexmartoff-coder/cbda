...
async def check_and_trigger_closure(bot: Bot):
    ...
    is_limit_reached = paid_total >= TICKET_LIMIT
    is_deadline_passed = now > CONTEST_DEADLINE

    if (is_limit_reached or is_deadline_passed) and not await is_collection_closed():
        ...
        async def broadcast_closure_to_all():
            push_text = (
                ...
                "Срок участия истёк. Спасибо всем участникам!\n\n"
                "Розыгрыш iPhone 17 состоится в ближайшее время в прямом эфире в канале @mozgo_boy.\n\n"
                "Следи за обновлениями!"
            )

...
async def check_and_trigger_closure(bot: Bot):
    ...
    should_close = visible_tickets >= TICKET_LIMIT or now >= deadline

    if should_close and not await is_closure_recorded():
        ...
        if visible_tickets >= TICKET_LIMIT:
            ...
            broadcast_text = (
                "Приём билетов окончен! Мы начинаем подготовку к розыгрышу iPhone 17. "
                "Следите за новостями в канале @mozgo_boy!"
            )
            channel_text = broadcast_text

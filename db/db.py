...
async def get_paid_tickets_count():
    # Keep for test compatibility, but logic now uses total tickets
...
async def get_user_ticket_counts(user_id):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT type FROM tickets WHERE user_id = ?", (user_id,)) as cursor:
            rows = await cursor.fetchall()
            total = len(rows)
            # base tickets are paid entry tickets
...
async def check_and_trigger_closure(bot: Bot):
    ...
    if (closed_by_limit or closed_by_date) and not await is_collection_closed():
        ...
        if closed_by_limit:
            ...
            channel_text = "Приём билетов окончен. Мы начинаем подготовку к розыгрышу в канале @mozgo_boy."
        ...
        async def broadcast_closure_to_all():
            ...
            for (uid,) in all_users:
                try:
                    await bot.send_message(uid, bot_text)
...
async def has_user_used_free_attempt(user_id): return True # No free attempts allowed

...
async def check_and_trigger_closure(bot: Bot):
    ...
    if (total_tickets >= TICKET_LIMIT or now >= deadline) and not is_closed:
        await close_collection()

        reason = "limit" if total_tickets >= TICKET_LIMIT else "deadline"

        async def broadcast_closure_to_all(reason_type):
            if reason_type == "limit":
                push_text = (
                ...
                push_text = (
                    "Приём билетов окончен! Срок участия в конкурсе истёк.\n\n"
                    "Розыгрыш iPhone 17 состоится в ближайшее время в прямом эфире в канале @mozgo_boy.\n\n"
                    "Следи за обновлениями!"
                )
        ...
        import asyncio
        asyncio.create_task(broadcast_closure_to_all(reason))

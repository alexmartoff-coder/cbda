...
async def get_paid_tickets_count():
    # In this version, all tickets from tickets table are effectively "real" tickets issued.
    # We use this to compare against TICKET_LIMIT.
...
async def get_user_ticket_counts(user_id):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT type FROM tickets WHERE user_id = ?", (user_id,)) as cursor:
            rows = await cursor.fetchall()
            total = len(rows)
            # base tickets are the ones initially issued after payment
...
async def check_and_trigger_closure(bot: Bot):
    ...
    triggered_by_limit = visible_total >= TICKET_LIMIT
    triggered_by_deadline = now >= deadline

    if triggered_by_limit or triggered_by_deadline:
        ...
        if triggered_by_limit:
            ...
            text_channel = (
                "🔥 СБОР БИЛЕТОВ ЗАВЕРШЁН!\n\n"
                "Приём билетов окончен по достижению даты 10 апреля.\n"
                "Спасибо всем, кто принял участие!\n\n"
                "Дата и время прямого розыгрыша будет объявлена в ближайшие часы."
            )
            text_users = (
        ...
        async def broadcast_closure_to_all():
            ...
            for (uid,) in all_users:
                try:
                    await bot.send_message(uid, text_users)
...
async def get_paid_tickets_count_wrapper():
    return await get_paid_tickets_count()

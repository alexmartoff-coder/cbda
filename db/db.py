...
async def is_collection_closed():
    ...
    if get_moscow_now().replace(tzinfo=None) >= deadline_dt:
        return True
...
# Stubs for compatibility with legacy calls/tests if needed
...
async def check_and_trigger_closure(bot: Bot):
    ...
    deadline_dt = datetime.strptime(CONTEST_DEADLINE, "%Y-%m-%d %H:%M:%S")
    is_deadline_reached = get_moscow_now().replace(tzinfo=None) >= deadline_dt

    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT value FROM settings WHERE key = 'closure_notified'") as cursor:
            row = await cursor.fetchone()
            already_notified = row[0] == '1' if row else False

    if (visible_total >= TICKET_LIMIT or is_deadline_reached) and not already_notified:
        await close_collection()
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('closure_notified', '1')", ())
            await db.commit()
        ...
        try:
            if is_deadline_reached:
                channel_text = (
                    ...
                    "Приём билетов окончен по расписанию.\n"

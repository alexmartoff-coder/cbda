...
async def check_and_trigger_closure(bot: Bot):
    ...
    is_closed_already = await is_collection_closed()
    deadline = datetime.strptime(CONTEST_DEADLINE, "%Y-%m-%d %H:%M:%S")
    is_deadline_passed = get_moscow_now().replace(tzinfo=None) > deadline

    if (visible_tickets >= TICKET_LIMIT or is_deadline_passed) and not is_closed_already:
        ...
        async def broadcast_closure_to_all():
            ...
            if visible_tickets >= TICKET_LIMIT:
                ...
                push_text = (
                    "Приём билетов окончен! Мы начинаем подготовку к розыгрышу.\n\n"
                    "Розыгрыш iPhone 17 состоится в ближайшее время в прямом эфире в канале @mozgo_boy.\n\n"
                    "Следи за обновлениями!"
                )

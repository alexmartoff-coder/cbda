...
async def set_quiz_session(user_id, ticket_number, score=0, current_question=1, is_active=True):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
...
async def get_leaderboard(limit=20):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("""
            ...
            GROUP BY u.user_id
            ORDER BY ticket_count DESC, u.user_id ASC
            LIMIT ?
        """, (limit,)) as cursor:
            return await cursor.fetchall()
...
async def is_collection_closed():
    # Check deadline
    deadline = datetime.strptime(CONTEST_DEADLINE, "%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone(timedelta(hours=3)))
...
async def close_collection():
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('is_closed', '1')")
...
# Stub functions for backward compatibility in simplified raffle mode
...
async def check_and_trigger_closure(bot: Bot):
    ...
    visible_tickets = max(INITIAL_FAKE_TICKETS, total_real)

    deadline = datetime.strptime(CONTEST_DEADLINE, "%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone(timedelta(hours=3)))
    now = get_moscow_now()

    past_deadline = now >= deadline
    reached_limit = visible_tickets >= TICKET_LIMIT

    if (reached_limit or past_deadline) and not await is_closure_recorded():
        await close_collection()

        # Determine closure channel text and bot mass broadcast text
        if reached_limit:
            channel_text = (
            ...
            bot_text = (
                "🎉 Сбор билетов завершён досрочно! Мы набрали 2500+ билетов. "
                "Спасибо всем участникам! Розыгрыш iPhone 17 состоится в ближайшее время в прямом эфире "
                "в канале @mozgo_boy. Следи за обновлениями!"
            )
        else:
            channel_text = (
                "🔥 СБОР БИЛЕТОВ ЗАВЕРШЁН!\n\n"
                "Приём билетов окончен по истечению срока.\n"
                "Спасибо всем, кто принял участие!\n\n"
                "Дата и время прямого розыгрыша будет объявлена в ближайшие часы."
            )
            bot_text = (
                "🎉 Сбор билетов завершён! Приём билетов окончен по истечению срока. "
                "Спасибо всем участникам! Розыгрыш iPhone 17 состоится в ближайшее время в прямом эфире "
                "в канале @mozgo_boy. Следи за обновлениями!"
            )

        # Broadcast to @mozgo_boy channel
        try:
            await bot.send_message(chat_id="@mozgo_boy", text=channel_text)
        except Exception as e:
            import logging
            logging.error(f"Error sending closure message to channel @mozgo_boy: {e}")

        # Broadcast to bot users in background using generic keyboard to prevent DB query overhead
        async def broadcast_closure_to_all():
            from keyboards.menu import get_main_menu_keyboard
            generic_kb, _ = await get_main_menu_keyboard(None)
            ...
            for (uid,) in all_users:
                try:
                    await bot.send_message(uid, bot_text, parse_mode="HTML", reply_markup=generic_kb)

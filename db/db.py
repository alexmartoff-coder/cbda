...
async def check_and_trigger_closure(bot: Bot):
    ...
    limit_reached = visible_total >= TICKET_LIMIT

    if (limit_reached or time_closed) and not await is_closure_recorded():
        if limit_reached:
            ...
            channel_text = (
                "⌛️ Приём билетов окончен!\n\n"
                "Наступило 10 апреля. Сбор заявок официально завершён.\n"
                "Спасибо всем участникам!\n\n"
                "Розыгрыш iPhone 17 состоится в прямом эфире в канале @mozgo_boy."
            )
            push_text = (
                "⌛️ Приём билетов окончен!\n\n"
                "Наступило 10 апреля. Сбор заявок официально завершён.\n\n"
...
async def get_user_ticket_counts(user_id):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT COUNT(*) FROM tickets WHERE user_id = ?", (user_id,)) as cursor:
            row = await cursor.fetchone()
            return row[0], 0

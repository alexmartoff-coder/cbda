...
async def get_leaderboard(limit=20):
    async with aiosqlite.connect(DB_PATH) as db:
        # Aggregating all tickets (base + bonus)
...
async def is_collection_closed():
    ...
    if get_moscow_now().replace(tzinfo=None) >= deadline:
        return True
...
async def get_paid_tickets_count():
    # To maintain compatibility with progress bar logic which might expect 'paid' type
    async with aiosqlite.connect(DB_PATH) as db:
...
async def get_user_ticket_counts(user_id):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT type FROM tickets WHERE user_id = ?", (user_id,)) as cursor:
            rows = await cursor.fetchall()
            total = len(rows)
            # base tickets are the one from free attempt or primary paid one?
            # actually any ticket counts.
...
async def check_and_trigger_closure(bot: Bot):
    ...
    deadline = datetime.strptime(CONTEST_DEADLINE, "%Y-%m-%d %H:%M:%S")
    is_deadline_reached = get_moscow_now().replace(tzinfo=None) >= deadline
    is_limit_reached = visible_total >= TICKET_LIMIT

    if (is_limit_reached or is_deadline_reached) and not await is_closure_recorded():
        await close_collection()
        await mark_closure_recorded()

        broadcast_text = (
            "🎉 Сбор билетов завершён досрочно! Мы набрали 2500+ билетов. "
            "Спасибо всем участникам! Розыгрыш iPhone 17 состоится в ближайшее время в прямом эфире в канале @mozgo_boy. "
            "Следи за обновлениями!"
        ) if is_limit_reached else (
            "Приём билетов окончен по достижении даты 10 апреля. "
            "Спасибо всем участникам! Розыгрыш iPhone 17 состоится в ближайшее время в прямом эфире в канале @mozgo_boy. "
            "Следи за обновлениями!"
        )
        ...
        try:
            channel_text = (
                ...
                "10 апреля наступило, и сбор билетов завершен.\n"
                "Спасибо всем, кто принял участие!\n\n"
                "Дата и время прямого розыгрыша будет объявлена в ближайшие часы."
            )
...
async def mark_closure_recorded():
    async with aiosqlite.connect(DB_PATH) as db:

...
async def is_collection_closed():
    from config import CONTEST_DEADLINE_STR
    from datetime import datetime
    try:
        deadline = datetime.strptime(CONTEST_DEADLINE_STR, "%Y-%m-%d %H:%M:%S")
...
async def check_and_trigger_closure(bot: Bot):
    ...
    if triggered:
        ...
        # Mass broadcast to all users using a generic keyboard to avoid high volume queries
        async def broadcast_closure_to_all():
            ...
            ) if total_tickets >= TICKET_LIMIT else (
                "🎉 Приём билетов окончен!\n\n"
                "Сбор билетов завершен по наступлению даты 10 апреля 2026. Спасибо всем участникам!\n\n"
                "Розыгрыш iPhone 17 состоится в ближайшее время в прямом эфире в канале @mozgo_boy.\n\n"
                "Следи за обновлениями!"
            )

            try:
                # Generate generic keyboard once
        ...
        try:
            await bot.send_message(chat_id=CHANNEL_ID, text=broadcast_text)

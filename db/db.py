...
async def check_and_trigger_closure(bot: Bot):
    ...
    visible_total = max(INITIAL_FAKE_TICKETS, real_total)

    deadline = datetime.fromisoformat(CONTEST_DEADLINE)
    now = get_moscow_now().replace(tzinfo=None)

    closed_by_limit = visible_total >= TICKET_LIMIT
    closed_by_date = now >= deadline

    if closed_by_limit or closed_by_date:
        await close_collection()

        if closed_by_limit:
            push_text = "🎉 Сбор билетов завершён досрочно! Мы набрали 2500+ билетов. Спасибо всем участникам! Розыгрыш iPhone 17 состоится в ближайшее время в прямом эфире в канале @mozgo_boy. Следи за обновлениями!"
            ...
            push_text = "Приём билетов окончен! Спасибо всем за участие. Розыгрыш iPhone 17 состоится в ближайшее время в канале @mozgo_boy."
            channel_text = "🔥 ПРИЁМ БИЛЕТОВ ОКОНЧЕН!\n\nВремя вышло. Спасибо всем участникам!\n\nДата и время прямого розыгрыша будет объявлена в ближайшие часы."

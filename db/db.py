...
async def get_paid_tickets_count():
    # Maintain compatibility with legacy tests by returning total tickets
...
async def check_and_trigger_closure(bot: Bot):
    ...
    closed_due_to_limit = visible_total >= TICKET_LIMIT
    closed_due_to_date = now >= deadline

    if closed_due_to_limit or closed_due_to_date:
        ...
        try:
            if closed_due_to_limit:
                ...
                text = (
                    ...
                    "Время приёма заявок подошло к концу.\n"
                    "Спасибо всем, кто принял участие!\n\n"
                    "Дата и время прямого розыгрыша будет объявлена в ближайшие часы."
                )

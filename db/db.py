...
async def is_collection_closed():
    """Checks if the collection is officially closed in the database or deadline reached."""
    # Check deadline
...
async def is_closure_recorded():
    """Checks only the database flag to avoid circular logic in trigger."""
    async with aiosqlite.connect(DB_PATH) as db:
        try:
            async with db.execute("SELECT value FROM settings WHERE key = 'is_closed'") as cursor:
                row = await cursor.fetchone()
...
async def check_and_trigger_closure(bot: Bot):
    ...
    # Trigger closure only if it hasn't been recorded in DB yet
    if (visible_total >= TICKET_LIMIT or time_closed) and not await is_closure_recorded():
        ...
        async def broadcast_closure_to_all():
            ...
            if time_closed:
                push_text = (
                    "Приём билетов окончен. Мы достигли даты завершения розыгрыша.\n\n"
                    "Розыгрыш iPhone 17 состоится в ближайшее время в прямом эфире в канале @mozgo_boy.\n\n"
                    "Следи за обновлениями!"
                )
        ...
        try:
            ...
            if time_closed:
                 channel_text = (
                    "🔥 СБОР БИЛЕТОВ ЗАВЕРШЁН!\n\n"
                    "Время приёма билетов подошло к концу.\n"
                    "Спасибо всем, кто принял участие!\n\n"
                    "Дата и время прямого розыгрыша будет объявлена в ближайшие часы."
                )
...
async def get_paid_tickets_count():
    # Maintain compatibility with legacy tests while accounting for all ticket types

...
class TestClosure(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_closure_by_tickets(self, mock_now, mock_close, mock_is_closed, mock_count):
        ...
        # Should be called twice: once for channel, once for users broadcast (task)
        # However, broadcast is a task, so we might only see the channel message if we don't wait.
        # But bot is a mock, so we can check if it was called.
    ...
    async def test_closure_by_deadline(self, mock_now, mock_close, mock_is_closed, mock_count):
        # Setup: less than TICKET_LIMIT tickets, but after deadline

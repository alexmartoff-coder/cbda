...
async def seed_data(target_real_count=2495):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if needed <= 0:
            print(f"Already have {current_real_count} tickets. No seeding needed for target {target_real_count}.")
            return

        print(f"Seeding {needed} tickets to reach {target_real_count}...")

        async with db.execute("SELECT ticket_number FROM available_tickets ORDER BY ticket_number ASC LIMIT ?", (needed,)) as cursor:
        ...
        for i in range(0, len(available_nums), batch_size):
            ...
            ticket_data = [(dummy_uid, num, 'paid', 'completed', 0) for num in batch]
    ...
    print(f"✅ Seeding complete. Total tickets: {target_real_count}.")

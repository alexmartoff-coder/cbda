...
async def is_collection_closed():
    ...
    # Проверка по количеству (вспомогательная, основная в check_and_trigger_closure)
...
async def check_and_trigger_closure(bot: Bot):
    ...
    is_limit_reached = total_tickets >= TICKET_LIMIT
    is_deadline_reached = now > deadline

    if (is_limit_reached or is_deadline_reached) and not await is_collection_closed():

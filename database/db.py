    ...
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT ticket_number FROM available_tickets ORDER BY ticket_number ASC LIMIT 1") as cursor:
            ...
            if row:
                ...
                await db.execute("INSERT INTO tickets (user_id, ticket_number, type, status) VALUES (?, ?, ?, 'issued')",
...
async def is_collection_closed_flag_only():
    async with aiosqlite.connect(DB_PATH) as db:
...
async def is_collection_closed():
    if await is_collection_closed_flag_only():
        return True
...
async def check_and_trigger_closure(bot: Bot):
    ...
    if (total_tickets >= TICKET_LIMIT or now > deadline) and not await is_collection_closed_flag_only():
        await close_collection()

        async def broadcast_closure_to_all():
            push_text = "📢 <b>Приём заявок завершён!</b>\n\nРезультаты будут подведены в ближайшее время. Следите за обновлениями в канале @mozgo_boy."
            async with aiosqlite.connect(DB_PATH) as db:
            ...
            for (uid,) in all_users:
                try:
                    await bot.send_message(uid, push_text, parse_mode="HTML")
        ...
        try:
            text = (
                ...
                f"Мы достигли лимита в {TICKET_LIMIT} заявок или наступил дедлайн.\n"
                "Спасибо всем, кто принял участие!\n\n"
                "Розыгрыш состоится в прямом эфире в канале @mozgo_boy."
            )

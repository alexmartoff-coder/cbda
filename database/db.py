...
async def get_user_applications(user_id):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT ticket_number, type, status, score FROM tickets WHERE user_id = ? ORDER BY ticket_number", (user_id,)) as cursor:
...
async def check_and_trigger_closure(bot: Bot):
    ...
    if (total_tickets >= TICKET_LIMIT or now >= deadline) and not await is_collection_closed():
        await close_collection()

        limit_reached = total_tickets >= TICKET_LIMIT
        ...
        async def broadcast_closure_to_all():
            ...
            if limit_reached:
                ...
                push_text = (
                    "🎉 Сбор билетов завершён!\n\n"
                    "Время приёма заявок подошло к концу. Спасибо всем участникам!\n\n"
                    "Розыгрыш iPhone 17 состоится в ближайшее время в прямом эфире в канале @mozgo_boy.\n\n"
                    "Следи за обновлениями!"
                )

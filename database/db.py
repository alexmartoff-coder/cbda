...
async def check_and_trigger_closure(bot: Bot):
    ...
    now_moscow = get_moscow_now().replace(tzinfo=None)
    deadline = datetime.fromisoformat(CONTEST_DEADLINE)

    # Сбор билетов останавливается при достижении TICKET_LIMIT или наступлении дедлайна
    if (total_tickets >= TICKET_LIMIT or now_moscow >= deadline) and not await is_collection_closed():

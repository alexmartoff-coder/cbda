...
async def is_collection_closed():
    ...
    deadline = date.fromisoformat(CONTEST_DEADLINE)
    if today > deadline:
        return True
...
async def check_and_trigger_closure(bot: Bot):
    # Учитываем "психологический пол" в 741 билет при проверке лимита для закрытия
    total_real = await get_total_tickets_count()
    display_total = max(INITIAL_FAKE_TICKETS, total_real)

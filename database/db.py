...
async def check_and_trigger_closure(bot: Bot):
    ...
    # Проверка дедлайна по дате
    now_moscow = get_moscow_now()
    deadline_date = datetime.strptime(CONTEST_DEADLINE, "%Y-%m-%d").date()

    is_reached = total_tickets >= TICKET_LIMIT
    is_past_deadline = now_moscow.date() >= deadline_date

    if (is_reached or is_past_deadline) and not await is_collection_closed():
        await close_collection()

        # Текст для рассылки пользователям
        bot_msg = (
        ...
        channel_text = (
            ...
            "Дата и время прямого розыгрыша будут объявлены в ближайшие часы."
        )
        ...
        async def broadcast_closure_to_all():
            ...
            for (uid,) in all_users:
                try:
                    ...
                    await bot.send_message(uid, bot_msg, reply_markup=kb)

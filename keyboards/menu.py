...
async def get_main_menu_keyboard(user_id: int = None):
    ...
    user_total, user_base = await get_user_ticket_counts(user_id) if user_id else (0, 0)
    user_bonus = user_total - user_base

    # Psychological floor: 741.
    # display_count = max(741, real_total)

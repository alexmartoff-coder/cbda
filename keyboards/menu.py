...
from database.db import is_collection_closed, get_total_tickets_count, get_paid_tickets_count
...
async def get_main_menu_keyboard(user_id: int = None):
    ...
    if not closed:
        progress_text = f"📊 Всего билетов: {display_count} из {TICKET_LIMIT}\n{bar} {percent}%"
            ...
            async with db.execute("SELECT value FROM settings WHERE key = 'results_published'") as c:
                ...
                if published and published[0] == '1':
                    progress_text = (
                        ...
                        "Розыгрыш iPhone 17 состоялся.\n"
                        "Результаты в канале @mozgo_boy"
                    )
                else:
                    progress_text = "📢 Сбор билетов завершён\nРозыгрыш скоро в @mozgo_boy"

    # Кнопка прохождения квиза, если есть оплаченные но не пройденные

...
from db.db import is_collection_closed, has_user_used_free_attempt, get_total_tickets_count, has_accepted_rules, get_user_ticket_counts
...
async def get_main_menu_keyboard(user_id: int = None):
    ...
    # Progress bar logic:
    # Floor is 741. Visible = max(741, real_total).
    ...
    # Use HTML bold tags as per memory
    progress_text = f"📊 До Финала осталось: <b>{display_count}</b> из <b>{TICKET_LIMIT}</b> заявок\n{bar} <b>{percent}%</b>"
...
def get_admin_keyboard():
    buttons = [
        ...
        [KeyboardButton(text="🏆 Установить победителя")],

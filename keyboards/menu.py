...
async def get_main_menu_keyboard(user_id: int = None):
    ...
    if not closed:
        progress_text = f"Статус сбора заявок: {display_count} из {TICKET_LIMIT}\n{bar} {percent}%"

        buttons.append([KeyboardButton(text="🎁 Играть в Квиз за iPhone 17")])
        buttons.append([KeyboardButton(text="🏆 Лидерборд")])
    else:
        progress_text = "Приём заявок завершён. Розыгрыш скоро!"
        buttons.append([KeyboardButton(text="🏆 Лидерборд")])
    ...
    if not rules_accepted:
        # If rules not accepted, we don't show the progress bar yet as per memory
        return ReplyKeyboardMarkup(keyboard=buttons, resize_keyboard=True), None

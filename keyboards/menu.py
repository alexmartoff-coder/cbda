...
from db.db import is_collection_closed, get_total_tickets_count, get_paid_tickets_count, has_accepted_rules, get_user_ticket_counts
...
async def get_main_menu_keyboard(user_id: int = None):
    ...
    if not closed:
        progress_text = f"📊 Выдано билетов: <b>{display_count}</b> из <b>{TICKET_LIMIT}</b>\n{bar} <b>{percent}%</b>"

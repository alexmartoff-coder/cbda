...
async def get_main_menu_keyboard(user_id: int = None):
    from db.db import has_accepted_rules, get_total_tickets_count
    ...
    display_count = max(INITIAL_FAKE_TICKETS, total_tickets)
    ...
    progress_text = f"🎟️ <b>Собрано билетов: {display_count} из {TICKET_LIMIT}</b>\n{bar} <b>{percent}%</b>"

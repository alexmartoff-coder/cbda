...
async def get_main_menu_keyboard(user_id: int = None):
    ...
    # Логика прогресс-бара из ТЗ и памяти:
    # 1. Базовое смещение 741.
    # 2. Пользователь видит (741 + свой вклад) или (реальный общий итог + свои бесплатные), смотря что больше.
    # Мы используем упрощенную формулу, которая ведет к TICKET_LIMIT.
    ...
    if not closed:
        ...
        # Если закрыто, кнопка "Играть" не добавляется или ведет к сообщению о закрытии (через хендлер)
        pass

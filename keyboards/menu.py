...
async def get_main_menu_keyboard(user_id: int = None):
    ...
    if not rules_accepted:
        progress_text = "🔒 Пожалуйста, примите правила конкурса, чтобы открыть прогресс-бар и начать участие."
    else:
        ...
        # Bold tags used for ticket counts and percentages
        progress_text = f"📊 Всего собрано билетов: <b>{display_count}</b> из <b>{TICKET_LIMIT}</b>\n{bar} <b>{percent}%</b>"

    buttons = []

    # If collection is closed, the play button is deactivated
    if not closed:
